...
import { type IMultipleChoiceData, type INonChoiceData, type MultipleChoiceData, type SingleChoiceData } from 'types/database/choice'
...
export function validateChoiceData(value: unknown, validateValue: (value: unknown) => boolean): value is SingleChoiceData {
    return isRecord(value) && ('isChoice' in value
        ? ((value.isChoice === true && (!('value' in value) || (Array.isArray(value.value) && value.value.every(validateValue)))) ||
            (value.isChoice === false && (!('value' in value) || validateValue(value.value))))
        : !('value' in value) || validateValue(value.value))
}
